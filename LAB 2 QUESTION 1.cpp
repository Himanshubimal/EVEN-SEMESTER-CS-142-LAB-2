...
class node //to create a node class
{  public :
    int data;
    node *next;
};
...
void menu()  //menu of the functions that one can use
{ cout<<"\n This is the list of thing you can do using this program."<<endl;
 ...
 while (ch != 'x')  //condition : input is not x otherwise break the loop
   { menu();
     ...
     {   int post, data;
         ...
         else { cout<<" Invalid position. \n ";} //condition when the position entered is not valid for the linked list

     }
     else if (ch=='d')
     {
         l_l.delete_list();  //calling the delete_list() function
     }
     else if (ch=='e')
     {
         int post;
     ...
     {   int num;
         num = l_l.count_l();  //calling the count_l() function
         cout<<"\n There is ";
         cout<<num;
        ...
        {
             cout<<"\n You have entered an invalid key try again.";  //when someone enter a invalid character
        }
...
int link :: count_l()
{
    ...
    else if (pos->next == NULL) //when the number of element in the list is one
    {
        return 1;
    }
